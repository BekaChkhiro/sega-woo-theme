{{--
  ...
  Description: Displays products within a specific product category
  Inherits from archive-product but with category-specific enhancements
--}}
...
@section('page-header')
  <div class="mb-8">
    ...
    <div class="flex items-start gap-4">
      @php
        $currentCat = is_callable($currentCategory) ? $currentCategory() : $currentCategory;
        $thumbnailId = $currentCat ? get_term_meta($currentCat->term_id, 'thumbnail_id', true) : null;
        $thumbnailUrl = $thumbnailId ? wp_get_attachment_url($thumbnailId) : null;
      @endphp
      ...
      @if ($thumbnailUrl)
        <div class="hidden shrink-0 sm:block">
          <img
            src="{{ $thumbnailUrl }}"
            alt="{{ $shopTitle }}"
            class="h-16 w-16 rounded-lg object-cover ring-1 ring-secondary-200"
          >
        </div>
      @endif
    ...
    @if (!empty($subcategories))
      <div class="mt-6" x-data="subcategoryCarousel()">
        <div class="mb-3 flex items-center justify-between">
          <h3 class="text-sm font-semibold uppercase tracking-wide text-secondary-500">
            {{ __('Browse Subcategories', 'sega-woo-theme') }}
          </h3>
          {{-- Navigation Arrows --}}
          <div class="flex items-center gap-1">
            <button
              type="button"
              x-ref="prevBtn"
              class="flex h-8 w-8 items-center justify-center rounded-full border border-secondary-200 bg-white text-secondary-500 transition-all hover:border-primary-300 hover:bg-primary-50 hover:text-primary-600 disabled:cursor-not-allowed disabled:opacity-40"
              aria-label="{{ __('Previous', 'sega-woo-theme') }}"
            >
            ...
            <button
              ...
              x-ref="nextBtn"
              class="flex h-8 w-8 items-center justify-center rounded-full border border-secondary-200 bg-white text-secondary-500 transition-all hover:border-primary-300 hover:bg-primary-50 hover:text-primary-600 disabled:cursor-not-allowed disabled:opacity-40"
              aria-label="{{ __('Next', 'sega-woo-theme') }}"
            >
              <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="swiper" x-ref="swiperContainer">
          <div class="swiper-wrapper">
            @foreach ($subcategories as $subcat)
              @php
                $subcatThumbId = get_term_meta($subcat->term_id, 'thumbnail_id', true);
                $subcatThumbUrl = $subcatThumbId ? wp_get_attachment_image_url($subcatThumbId, 'thumbnail') : null;
              @endphp
              <div class="swiper-slide !w-auto">
                <a
                  href="{{ get_term_link($subcat) }}"
                  class="group inline-flex items-center gap-3 whitespace-nowrap rounded-full border border-secondary-200 bg-white py-1.5 pl-1.5 pr-4 text-sm font-medium text-secondary-700 shadow-sm transition-all duration-200 hover:border-primary-300 hover:bg-primary-50 hover:text-primary-700 hover:shadow-md"
                >
                  {{-- Category Thumbnail --}}
                  <span class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-secondary-100">
                    @if ($subcatThumbUrl)
                      <img
                        src="{{ $subcatThumbUrl }}"
                        alt="{{ $subcat->name }}"
                      ...
                      >
                    @else
                      <svg class="h-5 w-5 text-secondary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                      </svg>
                    @endif
                  </span>
                  <span>{{ $subcat->name }}</span>
                  <span class="rounded-full bg-secondary-100 px-2 py-0.5 text-xs font-semibold text-secondary-600 transition-colors group-hover:bg-primary-100 group-hover:text-primary-700">
                    {{ $subcat->count }}
                  </span>
                  <svg
                    class="h-4 w-4 text-secondary-400 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:text-primary-500"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2"
      ...
      @if (!empty($filtersArray))
        ...
        <div class="flex flex-wrap items-center gap-3">
          ...
          @if (count($filtersArray) > 1)
            @php
              // For category pages, "clear all" should go to the current category, not shop
              $clearUrl = $currentCat ? get_term_link($currentCat) : $shopUrl;
            @endphp
            <a
              href="{{ $clearUrl }}"
  ...
  @if ($hasProducts)
    ...
    <script>
      document.addEventListener('click', function(e) {
        ...
        // For category pages, include the current category
        @if ($currentCat)
          formData.append('categories[]', {{ $currentCat->term_id }});
        @endif
        ...
        fetch(window.sageShopAjax.ajaxUrl, { method: 'POST', body: formData })
          ...
          .then(data => {
            if (data.success) {
              ...
              else newUrl.searchParams.delete('paged');
  ...
  @else
    <div class="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p class="mb-6 max-w-sm text-secondary-600">
        ...
        @else
          {{ __('There are no products in this category yet. Please check back later.', 'sega-woo-theme') }}
        @endif
      </p>

      <div class="flex flex-wrap justify-center gap-3">
        @if ($isFiltered && $currentCat)
          <a
            href="{{ get_term_link($currentCat) }}"
          ...
          >
            {{ __('Clear filters', 'sega-woo-theme') }}
          </a>
        @endif
